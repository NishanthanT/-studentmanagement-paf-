const Toast = ({ message, type, onClose }) => {
    ...
    const icons = {
        success: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7" /></svg>,
        error: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>,
    ...
    return (
        <div className={`flex items-center gap-4 p-4 rounded-2xl shadow-2xl animate-slide-in font-bold text-sm border-2 border-white/20 backdrop-blur-md ${styles[type] || styles.info}`}>
            <div className="flex-shrink-0 bg-white/20 p-2 rounded-xl">
                {icons[type] || icons.info}
            </div>
            <div className="flex-grow tracking-tight uppercase whitespace-pre-wrap">{message}</div>
